...
const ProductFromData = ({ showActions, showCart }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Prepare weight options and price options from fetched data
        const allWeights = data.reduce((weights, product) => {
          product.weightAndPrice.forEach(item => {
        ...
        const allPrices = data.reduce((prices, product) => {
          product.weightAndPrice.forEach(item => {
        ...
        setWeightOptions(allWeights.map(weight => ({ value: weight, label: weight })));
        setPriceOptions(allPrices.map(price => ({ value: price, label: price })));
    ...
    <div className="relative bg-gradient-to-t from-gray-900/50 to-gray-900/25 pt-32 sm:pt-48 lg:pt-64">
      <div className="p-4 sm:p-6">
        ...
        <div className="flex justify-between mt-2">
          <div>
            <span className="font-semibold text-white/90">Weight:</span> {product.weightAndPrice && (
              product.weightAndPrice.map((item, index) => (
                <span key={index} className="text-xs text-white/90 ml-1">{item.weight}</span>
              ))
            )}
          </div>
          <div>
            <span className="font-semibold text-white/90">Price:</span> {product.weightAndPrice && (
              product.weightAndPrice.map((item, index) => (
                <span key={index} className="text-xs text-white/90 ml-1">{item.price}</span>
              ))
            )}
          </div>
        </div>
      ...
      {isEdit && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-md shadow-md">
            {/* Pass editProductData and handleSave as props to ProductForm */}
            <ProductForm initialValues={editProductData} handleSave={handleSave} isEdit={isEdit} weightOptions={weightOptions} priceOptions={priceOptions} />
            <button
